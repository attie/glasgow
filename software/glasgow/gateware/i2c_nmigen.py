...
class I2CInitiator(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.bus = bus = I2CBus(self.pads)
        ...
        def scl_h(state, next_state, sync_exprs=tuple()):
            with m.State(state):
                with m.If(stb):
                    m.d.sync += bus.scl_o.eq(1)
                with m.Elif(bus.scl_o != 1):
                    pass
                with m.Elif((not self.clk_stretch) | (bus.scl_i == 1)):
                    m.d.sync += sync_exprs
                    m.next = next_state

        def stb_x(state, next_state, sync_exprs=tuple(), cond_next_states=tuple()):
            with m.State(state):
                with m.If(stb):
                    m.d.sync += sync_exprs
                    m.next = next_state

                    for cond, state in cond_next_states:
                        with m.If(cond):
                            m.next = state
        ...
        with m.FSM() as fsm:
            ...
            scl_h("READ-DATA-SCL-H", "READ-DATA-SDA-N", (
                r_shreg.eq(Cat(bus.sda_i, r_shreg[:7])),
            ))
            stb_x("READ-DATA-SDA-N", "READ-DATA-SCL-L", (
                bitno.eq(bitno + 1),
            ),(
                ( (bitno == 7), "READ-ACK-SCL-L" ),
            ))
            # read ack
...
class I2CTarget(Elaboratable):
    ...
    """
    def __init__(self, pads):
        print('WARNING: gateware/i2c_nmigen.py IS BROKEN...')
    ...
    def elaborate(self, platform):
        ...
        with m.FSM() as fsm:
            ...
            with m.State("ADDR-SHIFT"):
                with m.If(bus.stop):
                    m.next = "IDLE"
                with m.Elif(bus.start):
                    m.next = "START"
                with m.Elif(bus.sample):
                    # TODO: confirm NextValue(shreg_i, (shreg_i << 1) | bus.sda_i),
                    m.d.sync += shreg_i.eq(Cat(bus.sda_i, shreg_i[:7]))
                with m.Elif(bus.setup):
            ...
            # WARNING: TODO: FSM.after_entering() doesn't exist in nMigen yet
            #m.d.comb += self.write.eq(fsm.after_entering("WRITE-ACK"))
            # two-step states are an attempt at an equivelant
            with m.State("WRITE-ACK"):
                m.d.comb += self.write.eq(1)
                m.next = "WRITE-ACK2"
            with m.State("WRITE-ACK2"):
                with m.If(bus.stop):
                    m.d.comb += self.stop.eq(1)
                    m.next = "IDLE"
                with m.Elif(bus.start):
                ...
                with m.Elif(bus.setup):
                    ...
                    m.nect = "WRITE-SHIFT"
                with m.Elif(~bus.scl_i):
                    m.d.sync += bus.scl_o.eq(~self.busy)
                    with m.If(self.ack_o):
                        m.d.sync += bus.sda_o.eq(0)

# WIP...

            # WARNING: TODO: FSM.before_entering() doesn't exist in nMigen yet
            #m.d.comb += self.read.eq(fsm.before_entering("READ-STRETCH"))
            # two-step states are an attempt at an equivelant
            with m.State("READ-STRETCH"):
                m.d.comb += self.read.eq(1)
                m.next = "READ-STRETCH-2"
            with m.State("READ-STRETCH-2"):
                with m.If(self.busy):
                    m.d.sync += shreg_o.eq(self.data_o)
                ...
                with m.Else():
                    with m.If(~bus.scl_i):
                        m.d.sync += bus.sda_o.eq(shreg_o[7])
                    m.d.sync += bus.scl_o.eq(1)
                    m.next = "READ-SHIFT"
            with m.State("READ-SHIFT"):
                ...
                with m.Elif(bus.sample):
                    m.d.sync += [
                        # TODO: confirm NextValue(shreg_o, shreg_o << 1),
                        shreg_o.eq(Cat(C(0,1), shreg_o[:7])),
                        bitno.eq(bitno + 1),
                    ]
                    with m.If(bitno == 7):
                        m.next = "READ-ACK"
